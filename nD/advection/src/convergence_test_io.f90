...
module convergence_test_io
  ...
  interface write_state
    module procedure write_state_2D
    module procedure write_state_3D
  end interface

  interface write_conv
    module procedure write_conv_2D
    module procedure write_conv_3D
  end interface
  ...
  contains
    ...
    subroutine read_param(fname, hl, skip_first_char)
      ...
      if (open_stat == 0) then
        ...
        if (present(skip_first_char) .and. skip_first_char .eqv. .true.) then
          ...
          if (ndims == 2) then
			read(20,*) desc, vel(1), vel(2)
		  else if (ndims == 3) then
			read(20,*) desc, vel(1), vel(2), vel(3)
		  end if
        ...
        ! skip header descripstion and read value line
          ...
          read(20,*) ndims, ncells, ngcells, tend, dt_max, cfl, vel, runs, &
            diff_const, scaling
        end if
    ...
    subroutine write_state_2D(state, folder, suffix_in)
      ! Task:
      ...
      fname = trim(fullname(folder)) // '/' // dim_str //'_advection_n-' // &
      ...
      if (open_stat == 0) then
        ...
        ! write out data
        do i = PHYmin, PHYmax
          write(unit=20,fmt=*, iostat=write_stat) state(PHYmin:PHYmax,i)
    ...
    subroutine write_state_3D(state, folder, suffix_in)
      ! Task:
      !   writes actual state into folder 'folder'.
      ! Input:
      !   state   -  current state,
      ...
      if (open_stat == 0) then
        ...
        do j = PHYmin, PHYmax
          do i = PHYmin, PHYmax
            write(unit=20,fmt=*, iostat=write_stat) state(PHYmin:PHYmax,i,j)
    ...
    end subroutine write_state_3D

    subroutine write_conv_2D(state, state_ref, folder, suffix_in)
      ! Task:
      ...
      !   suffix      -  optional, controls the data type [default = txt].
      ...
      real(dp), dimension(ncells+2*ngcells, ncells+2*ngcells), &
        intent(in) :: state, state_ref
      ...
      fname = trim(fullname(folder)) // dim_str //'_advection_' // 't-' &
      ...
      if (ex) then
	    ...
	    err = sum( abs(    state(PHYmin:PHYmax,PHYmin:PHYmax) - &
                     state_ref(PHYmin:PHYmax,PHYmin:PHYmax))  )

		err = err / real(ncells**ndims, dp)
    ...
    end subroutine write_conv_2D

    
    subroutine write_conv_3D(state, state_ref, folder, suffix_in)
      ! Task:
      !   writes out the number of cells 'ncells' and the error between initial
      !   state and actual state.
      ! Input:
      ...
      if (ex) then
	    ...
	    err = sum( abs(    state(PHYmin:PHYmax,PHYmin:PHYmax,PHYmin:PHYmax) - &
                     state_ref(PHYmin:PHYmax,PHYmin:PHYmax,PHYmin:PHYmax))  )
